/******************************************************************************
 *
 * $Id: main.c 56 2011-03-31 02:57:01Z yamada.rj $
 *
 * -- Copyright Notice --
 *
...
void Disp_MeasurementResultHook(AK8975PRMS* prms, const uint16 flag)
{
	if (!g_opmode) {
		...
		rbuf[5] = prms->m_avec.u.x;	/* Ax */
		rbuf[6] = prms->m_avec.u.y;	/* Ay */
		rbuf[7] = prms->m_avec.u.z;	/* Az */
		rbuf[8] = 3;				/* Acc status */
	...
	while((opt = getopt(argc, argv, "stm:z:")) != -1){
		switch(opt){
			...
			case 'z':
				...
				AKMDEBUG(DBG_LEVEL3, "%s: Dbg Zone=%d\n", __FUNCTION__, g_dbgzone);
				break;
			case 's':
				g_opmode = 1;
				break;
			case 't':
				g_dbforFactoryTest = 1;
				break;
			default:
	...
	// Here is the Main Loop
		...
		while (AKD_TRUE) {
			...
			switch (Menu_Main()) {
				...
				default:
					AKMDEBUG(DBG_LEVEL1, "Unknown operation mode.\n");
					break;
			}
		}
	} else {
